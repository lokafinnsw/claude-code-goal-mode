 ...
 *
 * v3.0.7: removed `tool_use_count` field — only existed to feed the removed
 * auto-pause-on-silence engagement check. Agent-dispatch scanning intact
 * (still required by the reviewer-independence guard).
 *
 * Returns `{ tokens_added, dispatches_added: [...] }`.
...
export function advanceCheckpoint(projectRoot, transcriptPath) {
  ...
  // v3.0.7: tool_use_count removed — only consumer (auto-pause-on-silence)
  // is gone. parseSlice still scans Agent dispatches for the reviewer-
  // independence guard, just no longer reports a per-tick tool count.
 ...
 *
 * v3.0.7: dropped `tool_use_count` from the return shape — only consumer
 * (auto-pause-on-silence engagement check) was removed.
 *
 * Returns `{ tokens, agents, rotated, checkpoint }`.
