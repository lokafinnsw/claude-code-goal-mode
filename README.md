...
- **v1.1.15** — fixed the autonomous loop in Desktop. Real-world finding (May 2026 runtime probe of `env` inside a claude-spawned subprocess from the Mac Desktop app): Desktop EMBEDS Claude Code (`~/Library/Application Support/Claude/claude-code/<ver>/claude.app/`) and runs the same hooks, plugins, and slash commands as the terminal `claude` command. Session id propagates as a CLI argument (`--resume <uuid>`), NOT as an env var, so `CLAUDE_CODE_SESSION_ID` is undefined in any subprocess the Desktop-spawned `claude` invokes (slash-command bash blocks, Bash tool, etc.). Our `start-goal-cli` previously errored on the missing env var. v1.1.15: if env var unset, fall back to `session_id="*"` (wildcard) which the Stop hook accepts as match-anything. Trade-off: if you run multiple Claude sessions of the same project simultaneously while a goal is active, all of them drive the goal — fine for single-user setups but loses multi-CLI-session isolation. CLI users with `claude` standalone, where the env var IS set, keep strict matching as before.
