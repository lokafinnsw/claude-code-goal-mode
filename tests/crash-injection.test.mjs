...
describe('crash A: state.json deleted between writes', () => {
  ...
  it('recoverCacheFromEvents rewrites state.json from events under lock (v2.0.3 explicit recovery API)', () => {
    const root = mkRoot();
    setupGoalWithEvents(root);
    fs.unlinkSync(statePath(root));
...
describe('crash B: tree.json deleted', () => {
  it('loadStateFromEvents reads via snapshot.tree (read-only after v2.0.3)', () => {
    const root = mkRoot();
    ...
    // v2.0.3: loadStateFromEvents is read-only. tree.json NOT regenerated by it.
    // Use recoverCacheFromEvents explicitly for cache-rewrite semantics.
    expect(fs.existsSync(treePath(root))).toBe(false);
    // Explicit cache-recovery path DOES regenerate it.
    recoverCacheFromEvents(root);
...
describe('crash C: both state.json AND tree.json corrupt', () => {
  it('snapshot + events alone reconstruct fully', () => {
    ...
    try {
      // Read via events (v2.0.3: read-only).
      const result = loadStateFromEvents(root);
      expect(result).toBeTruthy();
      expect(result.state.cursor).toBe('sprint-1.epic-1.task-2');
      expect(result.tree.root.id).toBe('sprint-1');
      // Explicit cache recovery: under lock, rewrite state.json + tree.json.
      const recovered = recoverCacheFromEvents(root);
      expect(recovered).toBeTruthy();
      // Subsequent legacy load now works.
      const fromCache = loadState(root);
      expect(fromCache.cursor).toBe('sprint-1.epic-1.task-2');
    } finally {
