...
**v3.0.9 — stable (`/goal-extend` now reopens `unmet` lifecycle + resets 3-NOGO-blocked cursor).** All foundational + v2-track work shipped. **v3.0.9** closes a recovery gap user-reported 2026-05-13: when a goal hit 3-strike NOGO escalation (`lifecycle=unmet`), `/goal-extend` previously rejected with `cannot extend budget from lifecycle=unmet`. The user had no in-place recovery — only `/goal-clear --archive` + re-plan from scratch, losing cursor, history, evidence, audits, and tree shape. `unmet` is now an `extend`-recoverable lifecycle (same shape as `budget-limited`), and when reopened from `unmet`, the engine auto-resets the blocking cursor's review-strike counter (status `blocked → pursuing`, review_attempts `3 → 0`, blocker_reason cleared) iff the cursor is unambiguously in the 3-NOGO escalation shape — other blocked states are left alone. Audit trail preserved via `budget-extended` history event with `transition: {from: 'unmet', to: 'pursuing'}` and `cursor_reset: {node_id, from_status, from_review_attempts, from_blocker_reason}`. 3-strike-NOGO canon in `apply-mutations.mjs` still applies on the next review cycle. **v3.0.8** closes a real feature gap: when a goal hits the triple-budget ceiling mid-run (e.g. 91/158 tasks done at 60M token cap), the only recovery pre-v3.0.8 was `/goal-clear --archive` + re-plan from scratch — destructive when the existing plan is still right and the user just wants more headroom. `goal-extend` bumps limits in-place (`--tokens +50M`, `--iter +1000`, `--time +4h`, with delta-vs-absolute modes) and auto-transitions `budget-limited → pursuing` so the Stop hook resumes on the existing plan. Triple budget enforcement unchanged. **v3.0.7** removes the auto-pause-on-silence detector entirely. It was a false-positive-prone heuristic originally added in v2.0.6 for the degenerate "controller refuses to engage" case; every patch (v3.0.5 threshold raise 5→20, v3.0.6 `tool_use` as engagement) reduced false-positives but never eliminated them on legitimate autonomous-controller runs. The triple budget (iterations / tokens / wall-clock) is the sole automatic safety net — it catches the real token-bleed case without false-positives on legitimate exploration phases. The `consecutive_silent_turns` state field is preserved as optional/default-0 for backward-compat; engine no longer reads or writes it. **v3.0.4** flips `stopHookDriver` default back to `true` — auto-drive (Stop-hook fires continuation on `pursuing`) is the out-of-the-box experience, matching the original product value: install, plan, walk away, come back. The v3 explicit CLI verbs (`evidence-add`, `achieve`, `submit-verdict`, `current`, `review-request`, `as-builtin`) remain fully supported and callable any time as opt-in tools. Hint-only mode (no auto-drive) is opt-in via `stopHookDriver: false`. **v3.0.1** adds a stale-review-pending detector: cursors stuck in `review-pending` >15min with no verdict events auto-transition to `awaiting-manual-approval`, preventing expensive Stop-hook prompt re-rendering when the controller stalls after dispatching a heavy reviewer subagent. **v3.0.2** is a brand rename only — surface labels updated to **Better Goal**; package name, plugin namespace, slash command prefix, and skill directories are unchanged (no migration needed). **v3.0.3** closes a CLI deadlock — `evidence-add` and `achieve` now auto-promote `cursor.status` from `pending → pursuing` on first engagement (was previously only set by v2 driver-mode tag emission), making v3 CLI verbs callable after `goal-resume` or on cursors left from historical v2 advance paths.
